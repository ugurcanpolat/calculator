...
int main (void) {
	...
	do { //use "do-while" to ask whether the user wants to continue or not
		if (result == 0 && tempResult == 0)	{ //if x = 0 ask new number to calculate
			printf ("Enter integer1: "); //get the number from user
			scanf ("%f", &x);
		}
		else { //if x != 0 use last result as x
			x = result;
		}
		...
		scanf (" %c", &opertr);
			...
			switch (opertr){ // Use a mechanism that converts 'operator' entered by user to  'operatorConverted'
...
double powerTaylor (float x, int y) { //power function for taylor series which uses in sine-cosine function
	double powX = x;
	for (; y>1; y--) powX *= x; 
	return powX;
}
